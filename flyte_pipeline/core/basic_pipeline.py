...
from core.validations import RawData, TargetSerie, TrainData
from core.utils.metrics import eval_metrics


TIMESTAMP = datetime.now().strftime('%Y%m%d%H%M%S')
MODEL_NAME = "diamonds_predictor"
LOCAL_IP = "<YOUR LOCALHOST IP HERE>"
...
mlflow.set_experiment("webinar-diamonds-training-serving-pipeline")
...
def evaluate_model(model: JoblibSerializedFile, x_test: DataFrame[TrainData], y_test: DataFrame[TargetSerie]) -> typing.Dict:
...
@task(requests=Resources(cpu="1", mem="1Gi"))
def validate_model(model_metrics: typing.Dict, new_model: JoblibSerializedFile, dataset: DataFrame[RawData]) -> typing.Dict:
    deploy_dict = {}
    target = "price"
...
def register_to_superwise(deploy_dict: dict,
    ...
    if len(models) == 0:
        ...
        diamond_model = Model(project_id=1, name=model_name, description="Predicting Diamond Prices")
...
@task
def deploy_model(delpoyment_dict: typing.Dict) -> typing.Dict:
    if not delpoyment_dict["to_deploy"]:
        return

    model_uri = f"{mlflow.get_run(delpoyment_dict['run_id']).to_dictionary()['info']['artifact_uri']}/{MODEL_NAME}/model.pkl"
    ...
    res = requests.post(f"http://{LOCAL_IP}:5050/diamonds/v1/update-model",
                        json={"model_uri":model_uri,
                              "model_id":delpoyment_dict["model_id"],
                              "version_id":delpoyment_dict["version_id"]})
...
def ml_pipeline(diamonds_price_threshold: int = 0):
    ...
    performance_metrics = evaluate_model(model=trained_model, x_test=x_test, y_test=y_test)
    delpoyment_dict = validate_model(model_metrics=performance_metrics, new_model=trained_model, dataset=validated_data)
    
    superwise_delpoyment_dict = register_to_superwise(deploy_dict=delpoyment_dict,
                                         model_name="Diamonds Price Predictor",
                                         baseline=validated_data)

    deploy_model(delpoyment_dict=superwise_delpoyment_dict)
